...
namespace ft {
	...
	public:
		tree_iterator(): _node(NULL), _flag(false) {};

		tree_iterator(void* node, bool _flag = false) : _node(static_cast<node_pointer>(node)), _flag(_flag) {
		}

		tree_iterator(const tree_iterator<typename remove_const<value_type>::type>& value) {
			_node = value._node;
			_flag = value._flag;
		}
		...
		tree_iterator& operator=(const tree_iterator<typename remove_const<value_type>::type>& value) {
			if (this != &value) {
		...
		tree_iterator operator++(int) {
			tree_iterator<value_type> new_it = *this;
			if (check_max(_node))
				_flag = true;
			else if (_node->_rnode != NULL)
				_node = min(_node->_rnode);
			else {
				node_pointer start = _node;
				node_pointer tmp;
				tmp = _node->_parent;
				while (tmp != NULL && _node == tmp->_rnode) {
					_node = tmp;
		...
		bool operator==(const tree_iterator<T> value) {
